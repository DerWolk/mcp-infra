...
async def read_resource(uri: str) -> str:
    ...
    if uri.startswith("file://"):
        result = await FileResourceProvider.get_file_resource(uri)
    elif uri.startswith("db://"):
        result = await DatabaseResourceProvider.get_table_resource(uri)
    elif uri.startswith("system://"):
        result = await SystemResourceProvider.get_system_resource(uri)
    else:
...
async def handle_messages(request):
    ...
    # This is where you'd process incoming messages
...
def create_app():
    ...
    web_app.router.add_post("/messages", handle_messages)
    return web_app
